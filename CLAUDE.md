...
- [ ] **No arbitrary bracket values** (`text-[11px]`, `shadow-[...]`, `grid-cols-[...]`, `w-[420px]`) — round to nearest stock, redesign, or drop the feature. **Exceptions (documented):**
  ...
  - `max-w-[calc(100vw-2rem)]` (HamburgerMenu.jsx dropdown) — the dropdown is rendered via React Portal at viewport-fixed coordinates, and must clamp to the viewport width minus a 32px safe margin so it doesn't overflow on narrow screens. `calc(100vw - 2rem)` isn't expressible as stock Tailwind (no max-width utility takes a calc input).
- [ ] **No hex colour literals anywhere in JSX or JS data constants.** Chart palettes, tag colours, repo colours all resolve DaisyUI semantic CSS variables at runtime via `getComputedStyle`. **Exceptions:**
  ...
  - `themes.js` fallback `#808080` — single safety-net hex returned by `getMetaColor()` when a theme name isn't in the meta-color map. Should never trigger in practice (every registered theme has a meta colour) but provides a non-empty default for the PWA `<meta name="theme-color">` tag.
...
- [ ] No inline `style={}` objects in JSX unless values are runtime-computed from data (progress-bar widths, runtime-computed container heights from data-row counts, portal positioning, Chart.js dataset colours resolved at runtime). **Exceptions for static inline styles (documented):**
  - **Root ErrorBoundary in `dashboard/js/main.jsx`** — the error fallback uses a small inline `style={}` block with static layout values (`minHeight: '100vh'`, centering flexbox, padding, gap) because it renders only after the app has thrown AND is the last defence when the Tailwind CSS bundle may have failed to load. Using utility classes would defeat the fallback purpose if the stylesheet is exactly what failed. Text colours inside the fallback still use DaisyUI semantic utilities (`text-base-content/80`) — if CSS loads they apply; if not, the browser default is still readable on the unstyled body. This is the ONLY "static inline style" exception outside the debug subsystem.
  - **DebugPill subsystem** — same rationale as the hex-literal exception above. All `style={}` in `DebugPill.jsx` and `components/debug/*` is allowed because the pill must render without Tailwind.
...
- `dashboard/` - React dashboard (Vite + React + Tailwind v4 + DaisyUI v5 + Chart.js via react-chartjs-2)
  ...
  - `js/AppContext.jsx` - React Context + useReducer state management (delegates all theme DOM mutations to `themes.js applyTheme()`)
