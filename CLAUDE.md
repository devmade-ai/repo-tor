...
- `scripts/aggregate-processed.js` - Aggregates processed/ data into time-windowed dashboard JSON (summary + per-month commit files + weekly/daily/monthly pre-aggregations). **Runs automatically on `npm run dev` and `npm run build`** — its output (`dashboard/public/data.json`, `dashboard/public/data-commits/`, `dashboard/public/repos/`) is gitignored as build artefact, same pattern as `scripts/write-build-version.mjs` / `dashboard/public/version.json`. Manual invocation still works but no longer required after pipeline changes — the next build regenerates everything.
