...
- [ ] **No arbitrary bracket values** (`text-[11px]`, `shadow-[...]`, `grid-cols-[...]`, `w-[420px]`) — round to nearest stock, redesign, or drop the feature. **Exceptions (documented):**
  ...
  - `grid-cols-[auto_repeat(7,1fr)]` (Timing.jsx full heatmap) — grid templates with mixed `auto` + `repeat()` aren't expressible as stock Tailwind, and row-alignment between the hour-label column and the 7 day columns is a functional data-correctness requirement, not cosmetic.
...
- [ ] **No hex colour literals anywhere in JSX or JS data constants.** Chart palettes, tag colours, repo colours all resolve DaisyUI semantic CSS variables at runtime via `getComputedStyle`. **Exceptions:**
  - `DebugPill.jsx` inline hex (~20 occurrences) — the debug pill renders in an isolated React root (`#debug-root`) that must survive App crashes AND stylesheet load failures. Tailwind classes won't reach this root if the main CSS bundle never loads, so the pill uses inline hex colours sourced from a fixed dark-terminal palette.
