...
- `scripts/generate-theme-meta.mjs` - Build-time oklch→hex converter. Reads each registered DaisyUI theme's `theme/*/object.js`, extracts `--color-base-100`, converts to hex, writes `dashboard/js/generated/themeMeta.js` with `META_COLORS` / `IS_DARK` / `THEME_NAMES`. Runs as the first step of `npm run dev` and `npm run build` so the PWA status bar hex values and the theme catalog never drift from DaisyUI's own oklch definitions.
- `dashboard/` - React dashboard (Vite + React + Tailwind v4 + DaisyUI v5 + Chart.js via react-chartjs-2)
  ...
  - `js/themes.js` - Theme catalog, validators (`validLightTheme`, `validDarkTheme`), storage helpers (`getStoredTheme`, `persistTheme`), and the single `applyTheme(dark, themeName, skipPersist)` helper that every theme-affecting caller routes through (AppContext darkMode effect, App.jsx embed override, cross-tab storage listener). Imports meta colors from `generated/themeMeta.js` which is regenerated by `scripts/generate-theme-meta.mjs` on every build.
