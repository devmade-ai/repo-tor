...
- [ ] **No arbitrary bracket values** (`text-[11px]`, `shadow-[...]`, `grid-cols-[...]`, `w-[420px]`) — round to nearest stock, redesign, or drop the feature. **Exceptions (documented):** design-token CSS var references (`z-[var(--z-sticky-header)]` in Header.jsx, `z-[var(--z-toast)]` in Toast.jsx) reference values that can't be stock because the project's z-index scale is a first-class design token in `:root`. The Timing.jsx full heatmap uses `grid-cols-[auto_repeat(7,1fr)]` because grid templates with mixed `auto` + `repeat()` aren't expressible as stock Tailwind and row-alignment between the hour-label column and the 7 day columns is a functional data-correctness requirement, not cosmetic.
