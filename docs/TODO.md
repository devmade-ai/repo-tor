...
*Last updated: 2026-04-13 — DaisyUI v5 component-class migration complete. All 10 phases shipped: modal (QuickGuide + InstallInstructionsModal), toast + alert, Health security alerts, Timeline work-pattern badges, card (unshadow .card), btn (unshadow .btn-*), TabBar tabs/tab class composition, form inputs (select + input), HamburgerMenu audit + hardcoded-color fix, FilterSidebar multi-select audit + checkbox migration. Combined with the prior full theming migration (8 registered themes, applyTheme() helper, single-source theme-config propagator), the dashboard now routes every user-visible surface through DaisyUI component classes with no custom classes shadowing DaisyUI's `@layer components` rules. See HISTORY.md for the full phase-by-phase changelog.*
