...
*Last updated: 2026-04-15 — two audit-cleanup passes complete on the `claude/migrate-daisyui-dark-mode-toG0Y` branch. First pass (13 commits, 4a8bd00–fd0985b): 18 findings, four section-component extractions (Timing, DebugPill, Discover, Timeline — every section now under the 500-line soft-limit), `HISTORY.md`/`HISTORY_ARCHIVE.md` split, `SESSION_NOTES.md` compact rewrite, `styles.css` trimmed 521→164 lines. Second pass (8 commits, 09ca333–5ecfad7): 20 fresh-eyes findings caught gaps in the first pass — CLAUDE.md exception list expanded for the debug subsystem + root ErrorBoundary, 9 more dead exports removed, `chartHeight` inline styles replaced with stock Tailwind utilities (11 call sites), body bg/color redundancy removed after verifying DaisyUI v5 base layer handles it, `AppContext.jsx` split into `AppContext.jsx` + `appReducer.js` (579→338+285 lines), Timeline `handleCardClick` micro-optimization. Zero custom CSS classes, zero known dead exports, every documented exception in CLAUDE.md has a per-entry rationale, every section + shared component file sits under the 500-line soft-limit. Source-level `node:test` tripwire is the only automated layer (60 tests, ~250ms). See HISTORY.md for the phase-by-phase changelog.*
